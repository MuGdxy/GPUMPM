...
bool SimulatorBuilder::build(MPMSimulator& simulator, int opt)
{
    ...
    if(opt == 0)
    {
        ...
        std::fill(simulator.h_vel.begin(),
                  simulator.h_vel.end(),
    ...
    char str[500];
    sprintf(str,
            "%s%s_initialize(%d)_%s_par[%d]_grid[%d]_%s_%e_",
            prefix.data(),
            getTransferScheme(transferScheme).data(),
            opt,
            getMaterialName(materialType).data(),
            simulator._numParticle,
            N,
            integratorType ? "imp" : "exp",
            dtDefault);
    simulator._fileTitle = std::string(str);

    ///
    reportMemory("before simulator_allocation");
    checkCudaErrors(cudaMalloc((void**)&simulator.d_memTrunk,
                               sizeof(T) * simulator._numParticle * (21)));  // tmp(21)
    checkCudaErrors(cudaMalloc((void**)&simulator.d_maxVelSquared, sizeof(T)));
    checkCudaErrors(cudaMalloc((void**)&simulator.d_masks, sizeof(uint64_t) * Dim));
    checkCudaErrors(cudaMemcpy(
        simulator.d_masks, simulator.h_masks.data(), sizeof(uint64_t) * Dim, cudaMemcpyHostToDevice));
    ...
    if(materialType == 0 || materialType == 1)
        simulator.buildModel(simulator.h_mass,
                             simulator.h_pos,
                             simulator.h_vel,
                             materialType,
                             youngsModulus,
