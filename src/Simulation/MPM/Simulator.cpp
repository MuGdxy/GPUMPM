                              ...
                              int                              idx)
{
    ...
    geometry->initialize(hmass.data(),
                         (const T*)hpos.data(),
                         (const T*)hvel.data(),
                         d_masks,
                         h_masks.data(),
...
void MPMSimulator::buildTransformer(const int gridVolume)
{
    ...
    transformer = std::make_unique<DomainTransformer<TEST_STRUCT<T>>>(_numParticle, gridVolume);
    transformer->initialize(d_masks, refGeometryPtr(0)->d_offsets, _tableSize, d_keyTable, d_valTable);
