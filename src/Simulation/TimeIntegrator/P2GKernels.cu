                        ...
                        uint64_t*   d_pageOffsets)
{
    ...
    uint32_t mark         = __ballot_sync(0xFFFFFFFF, bBoundary);  // a bit-mask
    mark                  = __brev(mark);
            ...
            for(int j = 0; j < 3; ++j)
            {
                for(int k = 0; k < 3; ++k)
                {
                    ...
                    for(int iter = 1; iter <= mark; iter <<= 1)
                    {
                        T tmp[4];
                        for(int i = 0; i < 4; ++i)
                            tmp[i] = __shfl_down_sync(0xFFFFFFFF, val[i], iter);
                        if(interval >= iter)
                            for(int i = 0; i < 4; ++i)
                                val[i] += tmp[i];
                    }
                    ...
                    if(bBoundary)
                        for(int ii = 0; ii < 4; ++ii)
                            atomicAdd(&(buffer[ii][smallest_node[0] + i]
        ...
        for(int i = 0; i < 3; ++i)
        {
            for(int j = 0; j < 3; ++j)
            {
                for(int k = 0; k < 3; ++k)
                {
                    ...
                    for(int iter = 1; iter <= mark; iter <<= 1)  // problem here MASK error
                    {
                        T tmp[7];
                        for(int i = 0; i < 7; ++i)
                        {
                            ...
                            tmp[i] = __shfl_down_sync(mask, val[i], iter);  // <- this mask error
                        }
                        if(interval >= iter)
                            for(int i = 0; i < 7; ++i)
                                val[i] += tmp[i];
    ...
    int**       d_adjPage)
{
    ...
    if(relParid < d_block_offsets[pageid + 1] - d_block_offsets[pageid])
    {
        ...
        T sig[9];
        sig[0] = d_sigma[parid_mapped + (0) * numParticle];
        sig[1] = d_sigma[parid_mapped + (1) * numParticle];
        sig[2] = d_sigma[parid_mapped + (2) * numParticle];
        sig[3] = d_sigma[parid_mapped + (3) * numParticle];
        sig[4] = d_sigma[parid_mapped + (4) * numParticle];
        sig[5] = d_sigma[parid_mapped + (5) * numParticle];
        sig[6] = d_sigma[parid_mapped + (6) * numParticle];
        sig[7] = d_sigma[parid_mapped + (7) * numParticle];
        sig[8] = d_sigma[parid_mapped + (8) * numParticle];
                                 ...
                                 T**         d_force)
{
    ...
    if(relParid < d_block_offsets[pageid + 1] - d_block_offsets[pageid])
    {
        ...
        for(int j = 0; j < 3; ++j)
        {
            for(int k = 0; k < 3; ++k)
            {
                ...
                val[0] = weight
                         * (dP[0] * xi_minus_xp[0] + dP[3] * xi_minus_xp[1]
                            + dP[6] * xi_minus_xp[2]);
                val[1] = weight
                         * (dP[1] * xi_minus_xp[0] + dP[4] * xi_minus_xp[1]
                            + dP[7] * xi_minus_xp[2]);
                val[2] = weight
                         * (dP[2] * xi_minus_xp[0] + dP[5] * xi_minus_xp[1]
                            + dP[8] * xi_minus_xp[2]);
                ...
                if(bBoundary)
                    for(int ii = 0; ii < 3; ++ii)
                        atomicAdd(&(buffer[ii][smallest_node[0]][smallest_node[1] + j][smallest_node[2] + k]),
                                  val[ii]);
            }
        }
        ...
        for(int j = 0; j < 3; ++j)
        {
            for(int k = 0; k < 3; ++k)
            {
                ...
                if(bBoundary)
                    for(int ii = 0; ii < 3; ++ii)
                        atomicAdd(&(buffer[ii][smallest_node[0] + 1]
                                          [smallest_node[1] + j][smallest_node[2] + k]),
                                  val[ii]);
            }
        }
        ...
        for(int j = 0; j < 3; ++j)
        {
            for(int k = 0; k < 3; ++k)
            {
                ...
                if(bBoundary)
                    for(int ii = 0; ii < 3; ++ii)
                        atomicAdd(&(buffer[ii][smallest_node[0] + 2]
                                          [smallest_node[1] + j][smallest_node[2] + k]),
                                  val[ii]);
            }
        }
    ...
    for(int ii = 0; ii < 3; ++ii)
        if(buffer[ii][bi * 4 + ci][bj * 4 + cj][bk * 4 + ck] != 0)
            atomicAdd((T*)((uint64_t)d_force[ii] + page_idx * 4096) + (ci * 16 + cj * 4 + ck),
                      buffer[ii][bi * 4 + ci][bj * 4 + cj][bk * 4 + ck]);
}
            ...
            for(int k = 0; k < 3; ++k)
            {
                ...
                for(int iter = 1; iter <= mark; iter <<= 1)
                {
                    ...
                    for(int i = 0; i < 3; ++i)
                    {
                        ...
                        tmp[i]    = __shfl_down_sync(mask, val[i], iter);
                    }
                    if(interval >= iter)
                        for(int i = 0; i < 3; ++i)
                            val[i] += tmp[i];
